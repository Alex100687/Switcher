...
public static class Injector
{
    ...
    private static readonly bool Diag = Environment.GetEnvironmentVariable("LAYOUTFIX_DEBUG") == "1";
    ...
    private static Thread StartThread()
    {
        ...
        }) { IsBackground = true, Name = "LayoutFix injector" };
        t.Start();
        return t;
    }
    ...
    {
        // Every injected event costs several ms on a machine with a few low-level hooks, so keep the count down:
        // backspaces as auto-repeat (N downs, one up) and unicode characters as key-downs with a single final up.
        var list = new List<Native.INPUT>(backspaces + text.Length + 6);
        if (backspaces > 0)
        {
            ushort bsScan = (ushort)Native.MapVirtualKeyEx(Native.VK_BACK, 0, IntPtr.Zero);
            for (int i = 0; i < backspaces; i++) list.Add(Key(Native.VK_BACK, bsScan, 0));
            list.Add(Key(Native.VK_BACK, bsScan, Native.KEYEVENTF_KEYUP));
        }
        char pendingUp = '\0';
        foreach (var ch in text)
        {
            ...
            if (ch == '\n' || ch == '\t')
            {
                if (pendingUp != '\0') { list.Add(Key(0, pendingUp, Native.KEYEVENTF_UNICODE | Native.KEYEVENTF_KEYUP)); pendingUp = '\0'; }
                AddVk(list, ch == '\n' ? Native.VK_RETURN : Native.VK_TAB);
                continue;
            }
            list.Add(Key(0, ch, Native.KEYEVENTF_UNICODE));
            pendingUp = ch;
        }
        if (pendingUp != '\0') list.Add(Key(0, pendingUp, Native.KEYEVENTF_UNICODE | Native.KEYEVENTF_KEYUP));
