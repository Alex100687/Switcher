...
{
    /// <summary>%AppData%\LayoutFix, or LAYOUTFIX_DATA_DIR if set (portable / testing).</summary>
    public static string Dir => Environment.GetEnvironmentVariable("LAYOUTFIX_DATA_DIR") is { Length: > 0 } d
        ? d
        : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "LayoutFix");
